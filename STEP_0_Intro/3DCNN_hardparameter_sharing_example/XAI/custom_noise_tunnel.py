...
class NoiseTunnel(Attribution):
    ...
    def __init__(self, attribution_method: Attribution) -> None:
        ...
        self.is_gradient_method = isinstance(self.attribution_method, GradientAttribution)
    ...
    def attribute(
        ...
        **kwargs: Any,
    ) -> Union[Union[Tensor, Tuple[Tensor, Tensor], Tuple[Tensor, ...], Tuple[Tuple[Tensor, ...], Tensor],]]:
        r"""
        Args:
        ...
        def add_noise_to_inputs(nt_samples_partition: int) -> Tuple[Tensor, ...]:
            if isinstance(stdevs, tuple):
                assert len(stdevs) == len(inputs), (
                    ...
                    "in {} must be equal to the number of stdevs values {}".format(len(inputs), len(stdevs))
                )
            else:
                assert isinstance(stdevs, float), "stdevs must be type float. " "Given: {}".format(type(stdevs))
        ...
        def add_noise_to_input(input: Tensor, stdev: float, nt_samples_partition: int) -> Tensor:
            # batch size
            bsz = input.shape[0]
            ...
            stdev_expanded = torch.tensor(stdev, device=input.device).repeat(input_expanded_size)
        ...
        def compute_partial_attribution(
            ...
            attributions = attr_func.__wrapped__(  # type: ignore
                ...
                inputs_with_noise_partition if is_inputs_tuple else inputs_with_noise_partition[0],
                **kwargs_partition,
            )
            delta = None
        ...
        def compute_smoothing(
            expected_attributions: Tuple[Union[Tensor], ...], expected_attributions_sq: Tuple[Union[Tensor], ...],
        ) -> Tuple[Tensor, ...]:
        ...
        def update_partial_attribution_and_delta(
            ...
            for i, attribution_partial in enumerate(attributions_partial):
                update_sum_attribution_and_sq(
                    sum_attributions, sum_attributions_sq, attribution_partial, i, nt_samples_partial,
                )
            if self.is_delta_supported and return_convergence_delta:
                delta_partial_list.append(delta_partial)

        return_convergence_delta: bool
        return_convergence_delta = "return_convergence_delta" in kwargs and kwargs["return_convergence_delta"]
        with torch.no_grad():
            nt_samples_batch_size = (
                nt_samples if nt_samples_batch_size is None else min(nt_samples, nt_samples_batch_size)
            )
            ...
            for _ in range(nt_samples_partition):
                ...
                (attributions_partial, is_attrib_tuple, delta_partial,) = compute_partial_attribution(
                    inputs_with_noise, kwargs_copy
                )
            ...
            nt_samples_remaining = nt_samples - nt_samples_partition * nt_samples_batch_size
            ...
            expected_attributions = tuple(
                [cast(Tensor, sum_attribution) * 1 / nt_samples for sum_attribution in sum_attributions]
            )
            expected_attributions_sq = tuple(
                [cast(Tensor, sum_attribution_sq) * 1 / nt_samples for sum_attribution_sq in sum_attributions_sq]
            )
            attributions = compute_smoothing(
                cast(Tuple[Tensor, ...], expected_attributions), cast(Tuple[Tensor, ...], expected_attributions_sq),
            )
    ...
    def _apply_checks_and_return_attributions(
        ...
        ret = cast(Union[TensorOrTupleOfTensorsGeneric, Tuple[TensorOrTupleOfTensorsGeneric, Tensor],], ret,)
        return ret
