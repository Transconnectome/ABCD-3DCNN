...
class MultiModalImageDataset(Dataset, Randomizable):
    ...
    def __getitem__(self, index: int):
        ...
        if self.image_only:
            ...
            img, meta_data = tuple(map(self.loader, self.image_files.iloc[index])) # modified
            if self.seg_files is not None:
        ...
        if self.transform is not None:
            ...
            if self.transform_with_metadata:
                img, meta_data = torch.stack((tuple(map(apply_transform, self.transform, img,
                                     repeat(False), repeat(True))))) # modified
            else:
                img = torch.stack((tuple(map(apply_transform, self.transform, img, repeat(False))))) # modified
...
class MultiChannelImageDataset(Dataset, Randomizable):
    ...
    def __getitem__(self, index: int):
        ...
        # load data and optionally meta
        if self.image_only:
            img = tuple(map(self.loader, self.image_files.iloc[index])) # modified
            if self.seg_files is not None:
                seg = self.loader(self.seg_files[index])
        else:
            img, meta_data = tuple(map(self.loader, self.image_files.iloc[index])) # modified
