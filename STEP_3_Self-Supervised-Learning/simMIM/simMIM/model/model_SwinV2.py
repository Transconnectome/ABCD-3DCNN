    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                pretrained_winow_size=pretrained_window_sizes[i_layer])
...
def swinV2_small_patch4_window8_3D(**kwargs):
    model = SwinTransformer3D(
        img_size=(128,128,128), patch_size=(4,4,4), depths=[2, 2, 18, 2], embed_dim=96, num_heads=[3, 6, 12, 24],              
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_path_rate=0.3, **kwargs)
    return model

def swinV2_base_patch4_window8_3D(**kwargs):
    model = SwinTransformer3D(
        img_size=(128,128,128), patch_size=(4,4,4), depths=[2, 2, 18, 2], embed_dim=128, num_heads=[4, 8, 16, 32],                 
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_path_rate=0.5, **kwargs)
    return model

def swinV2_large_patch4_window8_3D(**kwargs):
    model = SwinTransformer3D(
        img_size=(128,128,128), patch_size=(4,4,4), depths=[2, 2, 18, 2], embed_dim=192, num_heads=[6, 12, 24, 48],                
