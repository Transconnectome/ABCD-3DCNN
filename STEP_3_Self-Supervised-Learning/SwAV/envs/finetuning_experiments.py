...
def finetuning_experiment(partition, subject_data, save_dir, args): #in_channels,out_dim
    ...
    if args.resume == 'False':
        ...
        if args.checkpoint_dir != None:
            net, optimizer, scheduler, last_epoch, optimizer.param_groups[0]['lr'] = checkpoint_load(net, checkpoint_dir, optimizer, scheduler, args, mode='finetuing')
    ...
    for epoch in tqdm(range(last_epoch, args.epoch + last_epoch)):
        ...
        checkpoint_dir = checkpoint_save(net, optimizer, save_dir, epoch, scheduler, args, current_result=val_acc, previous_result=val_accs, mode='prediction')

    # test
    net.to('cpu')
    torch.cuda.empty_cache()

    net, _, _, _, _ = checkpoint_load(net, checkpoint_dir, optimizer, scheduler, args, mode ='eval')
    if args.sbatch == 'True':
        net.cuda()
    else:
