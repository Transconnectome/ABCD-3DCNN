...
#        decoder_embed_dim=576, decoder_depth=8, decoder_num_heads=16,
        decoder_embed_dim=1152, decoder_depth=12, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), spatial_dims=3, **kwargs)
    return model

def mae_vit_large_patch16_dec512d8b_3D(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=1536, depth=24, num_heads=16,              # original encoder embed_dim = 1024
        decoder_embed_dim=576, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), spatial_dims=3, **kwargs)
    return model
...
def mae_vit_huge_patch14_dec512d8b_3D(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=14, embed_dim=1920, depth=32, num_heads=16,              # original encoder embed_dim = 1280
        decoder_embed_dim=576, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), spatial_dims=3, **kwargs)
    return model
