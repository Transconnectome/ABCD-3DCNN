...
def checkpoint_save(net, optimizer, save_dir, epoch, args, current_result=None, previous_result=None, mode=None):
    ...
    if mode == 'simCLR':
        ...
        if best_checkpoint_votes == len(args.cat_target + args.num_target):
            torch.save({'backbone':net.module.backbone.state_dict(),
                        'FClayer':net.module.FClayer.state_dict(),
                        'optimizer':optimizer.state_dict(),
                        'epoch': epoch}, checkpoint_dir)
...
def checkpoint_load(net, checkpoint_dir, optimizer, mode='simCLR'):
    if mode == 'simCLR':
        ...
        print(model_state['projection_head'])
        net.backbone.load_state_dict(model_state['backbone'])
        net.projection_head(model_state['projection_head'])
        ...
        model_state = torch.load(checkpoint_dir, map_location='cpu')
        net.backbone.load_state_dict(model_state['backbone'])
        net.FClayer.load_state_dict(model_state['FClayer'])
        optimizer.load_state_dict(model_state['FClayer'])
        print('The last checkpoint is loaded')
        return net, optimizer, model_state['epoch']
...
def load_pretrained_model(net, checkpoint_dir):
    model_state = torch.load(checkpoint_dir, map_location = 'cpu')
    net.backbone.load_state_dict(model_state)
    print('The best checkpoint is loaded')
