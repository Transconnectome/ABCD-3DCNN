...
import models.prediction_model as prediction_model #model script

from envs.loss_functions import calculating_loss, calculating_acc
from utils.utils import argument_setting, CLIreporter, load_pretrained_model, save_exp_result, checkpoint_save, checkpoint_load, load_pretrained_model
...
def finetuning_experiment(partition, subject_data, args): #in_channels,out_dim
    ...
    if args.resume == False:
        ...
        if args.checkpoint_dir != None:
            net, optimizer, last_epoch = checkpoint_load(net, checkpoint_dir, optimizer, mode='finetuning')
        else:
    ...
    for epoch in tqdm(range(last_epoch, args.epoch)):
        ...
        checkpoint_dir = checkpoint_save(net, optimizer, checkpoint_dir, epoch, args, current_result=val_acc, previous_result=val_accs, mode='prediction')

    # test
    net.to('cpu')
    torch.cuda.empty_cache()

    net = checkpoint_load(net, checkpoint_dir, optimizer, mode ='eval')
    if args.sbatch == 'True':
        net.cuda()
    else:
