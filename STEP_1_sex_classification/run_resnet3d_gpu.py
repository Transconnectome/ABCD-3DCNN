...
parser.add_argument("--lr2", default=0.001,type=float,required=False,help='')
parser.add_argument("--weight_decay",default=0.001,type=float,required=False,help='')
#parser.add_argument("--lr",default=1e-5,type=float,required=False,help='') #wang
#parser.add_argument("--l2",default=0.00001,type=float,required=False,help='') #wang
...
base_dir = '/home/connectome/bettybetty3k/docker/share/preprocessed_masked'
...
def train(net,partition,optimizer,criterion,args):
    ...
    if args.optim == 'SAM':
        for i, data in enumerate(trainloader,0):
            image, label = data
            image = image.to(f'cuda:{net.device_ids[0]}') #image.cuda()
            label = label.to(f'cuda:{net.device_ids[0]}') #label.cuda()
...
def experiment(partition, args): #in_channels,out_dim
    ...
    if args.model == 'resnet3D50':
        ...
        net = torch.nn.DataParallel(net,device_ids=[7,6,5,4])
    ...
    elif args.model == 'ResNet3DRegressor':
        device = torch.device(f'cuda' if torch.cuda.is_available() else 'cpu')
        if torch.cuda.device_count() >1:
            print("Let's use", torch.cuda.device_count(), "GPUs!")

        net = resnet3d.ResNet3DRegressor
        net = torch.nn.DataParallel(net,device_ids=[7,6,5,4])
        #net.cuda()
        #net.cuda(1)
        net = net.to(f'cuda:{net.device_ids[0]}') #net = net.to(device)
    ...
    ##### PipelinedResNet3dRegressor #####
    elif args.model == 'PipelinedResNet3dRegressor':
        device = torch.device(f'cuda' if torch.cuda.is_available() else 'cpu')
        if torch.cuda.device_count() >1:
            print("Let's use", torch.cuda.device_count(), "GPUs!")

        net = resnet3d.PipelinedResNet3dRegressor(device=[7,6,5,4])
        ...
        criterion = nn.CrossEntropyLoss()
        if args.optim == 'SGD':
            optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=args.momentum) #lr=0.01, momentum=0.5)
        elif args.optim == 'Adam':
            optimizer = optim.Adam(net.parameters(),lr=args.lr2, weight_decay=args.weight_decay) #lr=0.001,weight_decay=0.001)
        else:
            raise ValueError('In-valid optimizer choice')
