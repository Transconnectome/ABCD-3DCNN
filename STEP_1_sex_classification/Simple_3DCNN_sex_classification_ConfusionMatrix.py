...
parser.add_argument("--GPU_NUM",default=1,type=int,required=False,help='')
...
class CNN3D_2(nn.Module):
    def __init__(self,in_channels,out_dim):
        ...
        self.classifier = nn.Sequential(nn.Linear(256*96*6*6,128),
                                        nn.BatchNorm1d(128),
                                        nn.Sigmoid(),
                                        nn.Dropout(),
...
def test(net,partition,args):
    ...
    for i, data in enumerate(testloader,0):
        ...
        total += label.size(0)
        correct += (predicted == label).sum().item()
        result_cmt = sklearn.metrics.confusion_matrix(label.cpu(),predicted.cpu())
        print(result_cmt)
    ...
    confusion_matrix['TrueNegative'] = true_negative
    confusion_matrix['FalsePositive'] =  false_positive
    confusion_matrix['FalseNegative'] = false_negative
    confusion_matrix['TruePositive'] = true_positive
...
def experiment(partition,args): #in_channels,out_dim
    if args.model == 'CNN3D_1':
        ...
        test_acc, confusion_matrix = test(net,partition,args)
        ...
        # saving model
        path = "/scratch/3DCNN/simpleCNN_results/{}-{}.pt".format(args.exp_name, args.epoch)
        torch.save((net.state_dict(),optimizer.state_dict()),path)

        return vars(args), result, confusion_matrix
...
setting, result, confusion_matrix = experiment(partition, deepcopy(args))
save_exp_result(setting,result, confusion_matrix)
## ==================================================== ##
