...
class DenseNet(nn.Module):
    ...
    def forward(self, x, y=None, target=None, manifold_mixup=False, lam=None):
        ...
        #features = torch.utils.checkpoint.checkpoint(self.conv_stem.conv0, x)
        features = self.conv_stem.norm0(features)
        #features = torch.utils.checkpoint.checkpoint(self.conv_stem.norm0,features)
        features = self.conv_stem.relu0(features)
        #features = torch.utils.checkpoint.checkpoint(self.conv_stem.relu0,features)
        features = self.conv_stem.pool0(features)
        #features = torch.utils.checkpoint.checkpoint(self.conv_stem.pool0,features)
        ...
        features = self.norm(features)
        #features = torch.utils.checkpoint.checkpoint(self.norm,features)
