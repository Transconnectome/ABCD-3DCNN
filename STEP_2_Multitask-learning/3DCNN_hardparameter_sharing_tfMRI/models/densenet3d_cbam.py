## =================================== ##
## ======= DensNet ======= ##
## =================================== ##

# model
import torch
import torch.nn as nn
import torch.nn.functional as F
#from torchsummary import summary
from torch import Tensor

from models.layers.CBAM import CBAM

## ========= DenseNet Model ========= #
#(ref) explanation - https://wingnim.tistory.com/39
#(ref) densenet3d - https://github.com/pytorch/vision/blob/main/torchvision/models/densenet.py
#(ref) pytorch - https://pytorch.org/vision/0.8/_modules/torchvision/models/densenet.html

#"""
class Conv3d(nn.Conv3d):
#"""
#"""
    """
    - convolution module with weight standardization 
    - Google brain paper (Big Transfer) report that pre-training with weight standardization + group norm significantly increase the model performance at finetuning
    1. weight standardization paper: arxiv.org/abs/1903.10520
    2. implementation: https://eehoeskrap.tistory.com/516
    3. pre-training with WS + group norm: https://arxiv.org/abs/1912.11370

    """
#"""
#"""
    def __init__(self, in_channels, output_channels, kernel_size, 
                stride=1, padding=0, dilation=1, groups=1, bias=True):
        super(Conv3d, self).__init__(in_channels, output_channels, kernel_size, stride, padding, dilation, groups, bias)

    def forward(self, x):
        w = self.weight
        w_mean = w.mean(dim=1, keepdim=True).mean(dim=2, keepdim=True).mean(dim=3, keepdim=True).mean(dim=4, keepdim=True)
        w = w - w_mean
        std = w.view(w.size(0), -1).std(dim=1).view(-1,1,1,1,1) + 1e-5
        w = w / std.expand_as(w)
        return F.conv3d(x, w, self.bias, self.stride, self.padding, self.dilation, self.groups)
#"""


class _DenseLayer(nn.Module):

    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        super().__init__()

        ## DenseNet Composite function: BN -> relu -> 3x3 conv
        # 1
        #self.add_module('norm1', nn.BatchNorm3d(num_input_features))
        self.add_module('norm1', nn.GroupNorm(num_groups=32, num_channels=num_input_features, eps=1e-6, affine=True))
        self.add_module('relu1', nn.ReLU(inplace=True))
        #self.add_module('conv1', nn.Conv3d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, bias=False))
        self.add_module('conv1', Conv3d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, bias=False))

        # 2
        #self.add_module('norm2', nn.BatchNorm3d(bn_size * growth_rate))
        self.add_module('norm2', nn.GroupNorm(num_groups=32, num_channels=bn_size * growth_rate, eps=1e-6, affine=True))
        self.add_module('relu2', nn.ReLU(inplace=True))
        #self.add_module('conv2', nn.Conv3d(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False))
        self.add_module('conv2', Conv3d(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False))
        
        self.drop_rate = float(drop_rate)
        #self.memory_efficient = memory_efficient
    
    def bn_function(self, inputs) -> Tensor:
        concated_features = torch.cat(inputs, 1)
        bottleneck_output = self.conv1(self.relu1(self.norm1(concated_features)))  # noqa: T484
        return bottleneck_output

    def forward(self, x):
        if isinstance(x, Tensor):
            prev_features = [x]
        else:
            prev_features = x

        bottleneck_output = self.bn_function(prev_features)
        new_features = self.conv2(self.relu2(self.norm2(bottleneck_output)))
        if self.drop_rate > 0: 
            new_features = F.dropout(new_features, p=self.drop_rate, training=self.training)
        return new_features  ## **

class _DenseBlock(nn.ModuleDict):
    # receive and concatenate the outputs of all previous blocks as inputs 
    # growth rate? the number of channel of feature map in each layer
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, use_cbam=True):
        super().__init__()
        
        for i in range(num_layers):
            if use_cbam: 
                layer = nn.Sequential(
                    _DenseLayer(num_input_features + i * growth_rate,
                                    growth_rate, bn_size, drop_rate),
                    CBAM(gate_channels=growth_rate, reduction_ratio=16)

                )
            else: 
                layer = _DenseLayer(num_input_features + i * growth_rate,
                                    growth_rate, bn_size, drop_rate)
            self.add_module("denselayer%d" % (i + 1), layer)
            
    
    def forward(self, init_features):
        features = [init_features]
        for name, layer in self.items():
            new_features = layer(features)
            features.append(new_features)
        return torch.cat(features, 1)

class _Transition(nn.Sequential):
    ## convolution + pooling between block
    # in paper: bach normalization -> 1x1 conv layer -> 2x2 average pooling layer

    def __init__(self, num_input_features, num_output_features):
        super().__init__()
        #self.add_module('norm', nn.BatchNorm3d(num_input_features))
        self.add_module('norm', nn.GroupNorm(num_groups=32, num_channels=num_input_features, eps=1e-6, affine=True))
        self.add_module('relu', nn.ReLU(inplace=True))
        #self.add_module('conv', nn.Conv3d(num_input_features, num_output_features, kernel_size=1, stride=1, bias=False))
        self.add_module('conv', Conv3d(num_input_features, num_output_features, kernel_size=1, stride=1, bias=False))
        self.add_module('pool', nn.AvgPool3d(kernel_size=2, stride=2))

class DenseNet_cbam(nn.Module):
    """Densenet-BC model class
    Args:
        growth_rate (int) - how many filters to add each layer (k in paper)
        block_config (list of 4 ints) - how many layers in each pooling block
        num_init_features (int) - the number of filters to learn in the first convolution layer
        bn_size (int) - multiplicative factor for number of bottle neck layers
          (i.e. bn_size * k features in the bottleneck layer)
        drop_rate (float) - dropout rate after each dense layer
        num_classes (int) - number of classification classes
    """

    def __init__(self, subject_data, args,
                 n_input_channels=1,conv1_t_size=7,conv1_t_stride=2,no_max_pool=False,
                 growth_rate=32,block_config=(6, 12, 24, 16),num_init_features=64,
                 bn_size=4,drop_rate=0.0,num_classes=1000):

        super(DenseNet_cbam, self).__init__()
        self.subject_data = subject_data
        self.cat_target = args.cat_target
        self.num_target = args.num_target 
        self.target = args.cat_target + args.num_target

        # First convolution
        self.conv_stem = nn.Module() 
        #self.conv_stem.add_module('conv0',nn.Conv3d(n_input_channels,
        self.conv_stem.add_module('conv0',Conv3d(n_input_channels,
                                num_init_features,
                                kernel_size=(conv1_t_size, 7, 7),
                                stride=(conv1_t_stride, 2, 2),
                                padding=(conv1_t_size // 2, 3, 3),
                                bias=False))
        #self.conv_stem.add_module('norm0', nn.BatchNorm3d(num_init_features))
        self.conv_stem.add_module('norm0', nn.GroupNorm(num_groups=32, num_channels=num_init_features, eps=1e-6, affine=True))
        self.conv_stem.add_module('relu0', nn.ReLU(inplace=True))
        self.conv_stem.add_module('pool0', nn.MaxPool3d(kernel_size=3, stride=2, padding=1))

        # Each denseblock
        self.num_features = num_init_features
        self.block_config = block_config
        self.growth_rate = growth_rate
        self.conv_features = nn.ModuleDict()
        #use_cbam = False 
        use_cbam = True
        for i, num_layers in enumerate(block_config):
            """
            if i >= len(block_config) - 2: 
                use_cbam = True
            """
            block = _DenseBlock(num_layers=num_layers,
                                num_input_features=self.num_features,
                                bn_size=bn_size,
                                growth_rate=growth_rate,
                                drop_rate=drop_rate,
                                use_cbam=use_cbam)
            self.conv_features.add_module('denseblock{}'.format(i + 1), block)
            self.num_features = self.num_features + num_layers * growth_rate
            cbam = CBAM(gate_channels=self.num_features)
            self.conv_features.add_module('cbam{}'.format(i + 1),  cbam)
        
            if i != len(block_config) - 1:
                """
                if i == len(block_config) - 2: 
                    cbam = CBAM(gate_channels=self.num_features)
                    self.conv_features.add_module('cbam{}'.format(i + 1),  cbam)
                """
                trans = _Transition(num_input_features=self.num_features,
                                    num_output_features=self.num_features // 2)
                
                self.conv_features.add_module('transition{}'.format(i + 1), trans)
                self.num_features = self.num_features // 2

        # Final batch norm
        #self.add_module('norm', nn.BatchNorm3d(self.num_features))
        self.add_module('norm', nn.GroupNorm(num_groups=32, num_channels=self.num_features, eps=1e-6, affine=True))

        # Linear layer
        self.FClayers = self._make_fclayers()

        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out',nonlinearity='relu')
            elif isinstance(m, nn.BatchNorm3d):
                nn.init.constant_(m.weight, 1)
                nn.init.constant_(m.bias, 0)
            elif isinstance(m, nn.Linear):
                nn.init.constant_(m.bias, 0)


    def _make_fclayers(self):
        FClayer = []
        
        for cat_label in self.cat_target:
            self.out_dim = len(self.subject_data[cat_label].value_counts())
            FClayer.append(nn.Sequential(nn.Linear(self.num_features, self.out_dim)))

        for num_label in self.num_target:
            FClayer.append(nn.Sequential(nn.Linear(self.num_features, 1)))

        return nn.ModuleList(FClayer)
    
    @torch.no_grad()
    def _hook_embeddings(self, x):
        features = self.conv_features(x)
        for name, layer in self.conv_features.items(): 
            features = layer(features)
        features = self.norm(features)
        out = F.relu(features, inplace=True)
        out = F.adaptive_avg_pool3d(out, output_size=(1, 1, 1))
        out = torch.flatten(out, 1)
        return out


    def forward(self, x, y=None, target=None, manifold_mixup=False, lam=None):
        results = {}
        # stem layers
        features = self.conv_stem.conv0(x)
        features = self.conv_stem.norm0(features)
        features = self.conv_stem.relu0(features)
        features = self.conv_stem.pool0(features)

        # dense layers 
        for name, layer in self.conv_features.items(): 
            features = layer(features)
        features = self.norm(features)

        # mixup process for manifold mixup
        if manifold_mixup: 
            """
            ref: https://github.com/vikasverma1077/manifold_mixup/blob/master/supervised/models/wide_resnet.py
            """
            assert lam is not None
            index = torch.randperm(x.shape[0]).to(x.device)
            # mixup process 
            features = lam.expand_as(features) * features + (1 - lam.expand_as(features)) * features[index]

        features = F.relu(features, inplace=True)
        features = F.adaptive_avg_pool3d(features, output_size=(1, 1, 1))
        features = torch.flatten(features, 1)
        # FC layers 
        for i in range(len(self.FClayers)):
            out = self.FClayers[i](features)
            if self.target[i] in self.cat_target:
                results[self.target[i]] = F.softmax(out,dim=1)
            else: 
                results[self.target[i]] = out

        if manifold_mixup: 
            assert y is not None 
            y_a , y_b= {}, {} 
            y_a[target], y_b[target] = y, y[index]
            return results, y_a, y_b 
        else:
            return results


def generate_model(model_depth, subject_data, args, **kwargs):
    assert model_depth in [121, 169, 201, 264]

    if model_depth == 121:
        model = DenseNet_cbam(subject_data, args,
                         num_init_features=64,
                         growth_rate=32,
                         block_config=(6, 12, 24, 16),
                         **kwargs)
    elif model_depth == 169:
        model = DenseNet_cbam(subject_data, args,
                         num_init_features=64,
                         growth_rate=32,
                         drop_rate=0.2, 
                         block_config=(6, 12, 32, 32),
                         **kwargs)
    elif model_depth == 201:
        model = DenseNet_cbam(subject_data, args,
                         num_init_features=64,
                         growth_rate=32,
                         drop_rate=0.2,
                         block_config=(6, 12, 48, 32),
                         **kwargs)
    elif model_depth == 264:
        model = DenseNet_cbam(subject_data, args,
                         num_init_features=64,
                         growth_rate=32,
                         block_config=(6, 12, 64, 48),
                         **kwargs)
    return model

def densenet3D121_cbam(subject_data, args):
    model = generate_model(121, subject_data, args)
    return model

def densenet3D169_cbam(subject_data, args):
    model = generate_model(169, subject_data, args)
    return model

def densenet3D201_cbam(subject_data, args):
    model = generate_model(201, subject_data, args)
    return model

def densenet3D264(subject_data, args):
    model = generate_model(264, subject_data, args)
    return model
