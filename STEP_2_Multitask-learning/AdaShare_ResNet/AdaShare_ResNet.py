...
class ResNet3d(nn.Module):
    ...
    def __init__(self, block, layers, subject_data, args, mode, num_classes=1000, zero_init_residual=False,
...
class MTL(nn.Module):
    ...
    """MTL_NAS는 curriculum learning의 첫번째 단계인 policy estimation을 위해서 필요한 단계이다. 즉, warmup 단계에서 쓰인다. 따라서 block dropping을 하지 않는다. """
    def __init__(self, block, layers, subject_data, args,init_method, mode = 'train',init_neg_logits=None, skip_layer=0):
        ...
        self.backbone = nn.DataParallel(ResNet3d(block,layers,subject_data, args, self.mode),device_ids=[1,2])
        self.layers = layers
    ...
    def forward(self, img, temperature, is_policy, num_train_layers=None, hard_sampling=False, mode='train'):
        ...
        for t_id in range(self.num_targets): # the order of tasks is cat_target~num_target
            ...
            results_policy['%s_policy' % self.targets[t_id]] = self.policys[t_id].detach().cpu().numpy().tolist() # get policy as list and stored in 'results'
        
        if mode == 'fix_policy':
            return results 
        else:
            return results, results_policy                    
...
def resnet3D50(subject_data,args):
    layers = [3, 4, 6, 3]
    model = MTL(Bottleneck3d, layers, subject_data, args, 'equal')
    return model
