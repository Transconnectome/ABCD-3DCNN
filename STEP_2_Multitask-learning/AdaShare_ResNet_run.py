...
base_dir = '/home/ubuntu/HEEHWAN/preprocessed_masked'
...
def train(net,partition,optimizer,input_dict,args):
...
def validate(net,partition,scheduler,input_dict,args):
...
def experiment(partition, subject_data, args): #in_channels,out_dim
    if args.model == 'resnet3D50':
        ...
        for epoch in tqdm(range(args.epoch)):
            ts = time.time()
            net, train_loss, train_acc = train(net,partition,optimizer,input_dict_train,args)
            val_loss, val_acc = validate(net,partition,scheduler,input_dict_val,args)
            te = time.time()
