...
class CNN3D(nn.Module):
    def __init__(self,in_channels,cat_target,num_target,subject_data):
        ...
        for cat_label in cat_target:
            ...
            self.fc = nn.Sequential(nn.Linear(6**3*256,19),
                                    nn.BatchNorm1d(19),
                                    nn.Sigmoid(),
                                    nn.Dropout(),
                                    nn.Linear(19,self.out_dim),
                                    nn.Softmax(dim=1))
        ...
        self.FClayer = nn.ModuleList(self.FClayer) # must store list of multihead fc layer as nn.ModuleList to attach FC layer to cuda
    ...
    def forward(self,x):
        ...
        for i in range(len(self.FClayer)):
            ...
            tmp_x = self.batchnorm1(tmp_x)
            #print(x.size())
            tmp_x = self.conv2(tmp_x)
            tmp_x = self.maxpool(tmp_x)
            tmp_x = self.batchnorm2(tmp_x)
            #print(x.size())
            tmp_x = self.conv3(tmp_x)
            tmp_x = self.conv4(tmp_x)
            tmp_x = self.maxpool(tmp_x)
            tmp_x = self.batchnorm3(tmp_x)
            #print(x.size())
            tmp_x = self.conv5(tmp_x)
            tmp_x = self.conv6(tmp_x)
            tmp_x = self.maxpool(tmp_x)
            #print(x.size())
            tmp_x = tmp_x.view(tmp_x.size(0),-1)
            #print(x.size())
            results[self.target[i]] = self.FClayer[i](tmp_x)
...
def calculating_loss_acc(targets, output, cat_target, num_target, correct, total, loss_dict, acc_dict):
    '''define calculating loss and accuracy function used during training step'''
    ...
    for num_label in num_target:
        ...
        tmp_loss = torch.sqrt(criterion(tmp_output.float(),y_true.float()))
        loss +=tmp_loss
                #print(tmp_loss)
...
def calculating_acc(targets, output, cat_target, num_target, correct, total, acc_dict):
...
def train(net,partition,cat_target,num_target,optimizer,args):
    ...
    if args.optim == 'SAM':
        for i, data in enumerate(trainloader,0):
            ...
            def closure():
                loss, correct, total, train_loss,train_acc = calculating_loss_acc(targets,output, args.cat_target, args.num_target, correct, total, train_loss, train_acc)


                loss.backward()
                return loss
            
                        # calculating loss from predicting each target variable separately. And sum all losses to backpropagate
            loss, correct, total, train_loss,train_acc = calculating_loss_acc(targets,output, args.cat_target, args.num_target, correct, total, train_loss, train_acc)
            loss.backward()
        ...
        for i, data in enumerate(trainloader,0):
            ...
            loss, correct, total, train_loss,train_acc = calculating_loss_acc(targets,output, args.cat_target, args.num_target, correct, total, train_loss, train_acc)
            loss.backward()                                  # multi-head model sum all the loss from predicting each target variable and back propagation
            optimizer.step()
...
def validate(net,partition,args):
    ...
    with torch.no_grad():
        for i, data in enumerate(valloader,0):
            ...
            loss, correct, total, val_loss,val_acc = calculating_loss_acc(targets,output, args.cat_target, args.num_target, correct, total, val_loss, val_acc)
...
def test(net,partition,args):
    ...
    for i, data in enumerate(testloader,0):
            ...
            correct, total, test_acc = calculating_acc(targets,output, args.cat_target, args.num_target, correct, total, test_acc)
...
## ========= Experiment =============== ##
def experiment(partition,cat_target,num_target,subject_data,args): #in_channels,out_dim
    ...
    for epoch in tqdm(range(args.epoch)):
        ts = time.time()
        net, train_loss, train_acc = train(net,partition,cat_target,num_target,optimizer,args)
        ...
        display(pd.DataFrame(visual_report, index=var_column))
...
def save_exp_result(setting, result, confusion_matrix):
...
setting, result, confusion_matrix = experiment(partition, subject_data,deepcopy(args))
